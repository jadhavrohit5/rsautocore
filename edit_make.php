...
#=================================================================
$title="Edit Make - Sony AutoParts";
$GLOBALS['message']="";
#==============================================================
...
$makeid = isset($_REQUEST['makeid']) ? base64_decode($_REQUEST['makeid']) : "";
$smarty->assign('makeid',$_REQUEST['makeid']); 
...
if(isset($_POST['pageaction']) && $_POST['pageaction'] == 'updmake') {
	$makenm = isset($_POST['makenm']) ? $_POST['makenm'] : "";

	$makeexists = pobe_chk_make_name($makeid,$makenm);

	if ($makeexists == 0){
		$dateposted = date("Y-m-d H:i:s");

		$ret_val3=$obj_custdt->update_partmakedata(array('name'=>addslashes($makenm), 'last_updated'=>$dateposted), array('id'=>$makeid));
/*--------------------------------------------------------------------------------*/
		header('location: manage_make.php?mode=show&msg='.base64_encode("Make data updated successfully."));
		die;
	} else {
...
{	
	...
	{		
		...
		$ret_val=$obj_custdt->get_partmakedata_details(array('id'=>addslashes($makeid)));
		$row_reqcnt=$obj_custdt->db->sql_fetchrowset();

		$makeid = stripslashes($row_reqcnt[0]['id']);	 
		$smarty->assign('makenm',stripslashes($row_reqcnt[0]['name']));

		$output=$smarty->fetch('edit_make.tpl');
