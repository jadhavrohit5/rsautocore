				...
				<div class="chart">
					<table>
						...
						<tbody>            
							...
							<tr class="{cycle values="odd,"} itemrow{$gsreqcnt[i].custid}" id="item-r{$gsreqcnt[i].cnt}">
								<td align="left" class="col-1" nowrap><span class="counter">{$gsreqcnt[i].cnt}</span></td>
								<td align="left">{$gsreqcnt[i].customernm} </td>
								<td align="center"><span class="action">
									<a href="edit_customer.php?mode=show&custid={$gsreqcnt[i].custid}" class="tooltip" title="Edit"><i class="fa fa-search"></i></a> 
									<a href="JavaScript:del('manage_customers.php?mode=show&act=delete&id={$gsreqcnt[i].custid}');" class="tooltip" title="Delete" ><i class="fa fa-trash-o"></i></a></span>
								</td>
							</tr>
							{/section} 
						</tbody>
