...
{/literal}
<div class="pageheading"> <a href="partslist.php?mode=show&type={$ptypeid}" class="tooltip back_btn" title="BACK"><i class="fa fa-angle-left"></i></a>
					...
					<table>
						<thead>
							<tr>
								...
								{if $hvGroups eq '1'}<th align="left"><a href="#">Stock</a></th>{/if}
								{if $hvGroups eq '0'}
						...
						<tbody>            
							...
							<tr class="{cycle values="odd,"} itemrow{$gsreqcnt[i].partid}" id="item-r{$gsreqcnt[i].cnt}">
								...
								<td align="center"><span class="action">
                                	{if $hvGroups eq '1'}<a href="update_item.php?type={$ptype}&partid={$gsreqcnt[i].partid}" class="tooltip" title="View Details"><i class="fa fa-search"></i></a>{else}<a href="update_part.php?type={$ptype}&partid={$gsreqcnt[i].partid}" class="tooltip" title="View Details"><i class="fa fa-search"></i></a>{/if}</span>
 	...
 	$(document).on('click', 'a.del', function(event){
		...
		var agree = confirm('Are you sure you want to delete this item?');
 		if(agree){  
			...
			var url = "partslist.php?mode=show&type={/literal}{$ptype}{literal}&act=delete&id="+id;
