...
<div class="pageheading"> <a href="parts.php?mode=show&type=1" class="tooltip back_btn" title="BACK"><i class="fa fa-angle-left"></i></a>
	<h1><i class="fa fa-cogs"></i> MANAGE MAKE</h1>
	<div class="add">
		<div class="search_form">
			<div class="adv_search_link"><a href="add_make.php?mode=show">Add New Make</a></div>
		</div>
	</div>
</div>
<div id="main_content">
					...
					<table>
						<thead>
							<tr>
								...
								<th align="left"><a href="#">Make Name </a></th>
								<th width="200" align="center">Actions</th>
							</tr>
						</thead>
						<tbody>            
							{section name=i loop=$gsreqcnt}
							<tr class="{cycle values="odd,"} itemrow{$gsreqcnt[i].makeid}" id="item-r{$gsreqcnt[i].cnt}">
								<td align="left" class="col-1" nowrap><span class="counter">{$gsreqcnt[i].cnt}</span></td>
								<td align="left">{$gsreqcnt[i].makenm} </td>
								<td align="center"><span class="action">
									<a href="edit_make.php?mode=show&makeid={$gsreqcnt[i].makeid}" class="tooltip" title="Edit"><i class="fa fa-search"></i></a> 
									<a href="JavaScript:del('manage_make.php?mode=show&act=delete&id={$gsreqcnt[i].makeid}');" class="tooltip" title="Delete" ><i class="fa fa-trash-o"></i></a></span>
								</td>
							</tr>
							{/section} 
						</tbody>
...
jQuery(document).ready(function ($) {
	//Delete item
 	$(document).on('click', 'a.del', function(event){
		...
		var agree = confirm('Are you sure you want to delete this item?');
 		if(agree){  
			...
			var url = "manage_make.php?mode=show&act=delete&id="+id;
	 ...
	 $("#page").change(function () {
        ...
        //alert(ppage);
  		var url = 'manage_make.php?mode=show&p='+ppage;
