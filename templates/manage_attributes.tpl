				...
				<div class="chart">
					<table>
						...
						<tbody>            
							...
							<tr class="{cycle values="odd,"} itemrow{$gsreqcnt[i].attrid}" id="item-r{$gsreqcnt[i].cnt}">
								<td align="left" class="col-1" nowrap><span class="counter">{$gsreqcnt[i].cnt}</span></td>
								<td align="left">{$gsreqcnt[i].attributenm} </td>
								<td align="center"><span class="action">
									<a href="edit_attribute.php?mode=show&attrid={$gsreqcnt[i].attrid}" class="tooltip" title="Edit"><i class="fa fa-search"></i></a> 
									<a href="JavaScript:del('manage_attributes.php?mode=show&act=delete&id={$gsreqcnt[i].attrid}');" class="tooltip" title="Delete" ><i class="fa fa-trash-o"></i></a></span>
								</td>
							</tr>
							{/section} 
						</tbody>
