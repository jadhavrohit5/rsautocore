...
//------------------------------------------------------------------------------------------------
	...
	*/
	...
	function pobe_get_attribute_data($aid,$pid)
	{
		...
		$rec = $db->fetch_array();
		$attributedata = stripslashes($rec['attrdata']);
...
//------------------------------------------------------------------------------------------------
	...
	//  added on 19-12-2022   
	function pobe_group_total_stock($grsacid)
	{
		$db = new build_sql();
		$db->query(" SELECT SUM(a_grade) as tot_stock FROM tbl_rsa_parts WHERE group_rsac = '" . addslashes($grsacid) . "' AND status = '1' AND is_deleted = '0' ");
		$rec = $db->fetch_array();
	...
	//  added on 19-12-2022   
	function pobe_group_target_stock($grsacid)
	{
		$db = new build_sql();
		$db->query(" SELECT SUM(target_stock) as tottarget FROM tbl_rsa_parts WHERE group_rsac = '" . addslashes($grsacid) . "' AND status = '1' AND is_deleted = '0' ");
		$rec = $db->fetch_array();
	...
	//  added on 19-12-2022   
	function pobe_group_parts_list($grsacid)
	{
		$db = new build_sql();					
		$db->query("SELECT trc.id, trc.rsac, trc.location as itemloc, trc.a_grade as itemqty, trp1.attrdata as itemoeone, trp2.attrdata as itemoetwo, trp3.attrdata as itemoemone, trp4.attrdata as itemoemtwo FROM tbl_rsa_parts trc  
		left join tbl_rsa_attributes_data trp1 on trp1.partid = trc.id and trp1.attrid = 50 
		left join tbl_rsa_attributes_data trp2 on trp2.partid = trc.id and trp2.attrid = 51 
		left join tbl_rsa_attributes_data trp3 on trp3.partid = trc.id and trp3.attrid = 52 
		left join tbl_rsa_attributes_data trp4 on trp4.partid = trc.id and trp4.attrid = 53 
		WHERE trc.group_rsac = '" . addslashes($grsacid) . "' AND trc.status = '1' AND trc.is_deleted = '0' ORDER BY trc.id ASC");
		...
		if($db->get_num_rows()) {
			$element = 0;
			while($clist = $db->fetch_array()){
				$grouppartslist[$element++] = array("pid" => stripslashes($clist['id']), "rsac" => stripslashes($clist['rsac']), "itemloc" => stripslashes($clist['itemloc']), "itemqty" => stripslashes($clist['itemqty']), "itemoeone" => stripslashes($clist['itemoeone']), "itemoetwo" => stripslashes($clist['itemoetwo']), "itemoemone" => stripslashes($clist['itemoemone']), "itemoemtwo" => stripslashes($clist['itemoemtwo']), "cnt" => $num);
				$num++;
			}
		}
	...
	//  added on 22-12-2022  - not used
	function pobe_get_prev_group($id,$ptype,$grsacid)
	{
		$db = new build_sql();
		$db->query(" SELECT id FROM tbl_rsa_parts WHERE status = '1' AND is_deleted = '0' AND part_type = '" . addslashes($ptype) . "' AND group_rsac != '".addslashes($grsacid)."' AND id < '" . addslashes($id) . "' order by group_rsac DESC LIMIT 1 ");
		$rec = $db->fetch_array();
		$previd = stripslashes($rec['id']);
		return $previd;
	}	
	...
	//  added on 22-12-2022  - not used   
	function pobe_get_next_group($id,$ptype,$grsacid)
	{
		$db = new build_sql();
		$db->query(" SELECT id FROM tbl_rsa_parts WHERE status = '1' AND is_deleted = '0' AND part_type = '" . addslashes($ptype) . "' AND group_rsac != '".addslashes($grsacid)."' AND id > '" . addslashes($id) . "' order by group_rsac ASC LIMIT 1 ");
		$rec = $db->fetch_array();
		$nextid = stripslashes($rec['id']);
		return $nextid;
	}	
