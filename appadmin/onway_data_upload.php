...
$uploadno = isset($_REQUEST['updno']) ? trim($_REQUEST['updno']) : "";
$smarty->assign('updno',$_REQUEST['updno']); 
...
{
	if($ACT=='deleteall' && $uploadno!=''){
		$datedeleted = date("Y-m-d H:i:s");
		$ret_vald4=$obj_partows->update_partsonway(array('datedeleted'=>$datedeleted, 'is_deleted'=>'1'), array('uploadnum'=>$uploadno,'is_deleted'=>'0'));
		$smarty->assign('msg',"Records deleted successfully.");
	}
	...
	{	
		$count=$obj_partows->count_partsonway_details(array('uploadnum'=>addslashes($uploadno),'status'=>'1','is_deleted'=>'0'));
		...
		$ret_cur=$obj_partows->get_partsonway($start,$limit,array('uploadnum'=>addslashes($uploadno),'status'=>'1','is_deleted'=>'0'));
		...
		if($ret_cur==1){
			...
			for($i=0;$i<count($row_webadm);$i++){
				...
				$gswebadm[$i]['poid']=base64_encode($row_webadm[$i]['id']);
				$gswebadm[$i]['ponum']=stripslashes($row_webadm[$i]['po_num']);
				$gswebadm[$i]['suppliername']=stripslashes($row_webadm[$i]['supplier']);
				$gswebadm[$i]['rsacno']=stripslashes($row_webadm[$i]['rsac_ref']);
				$gswebadm[$i]['quantity']=stripslashes($row_webadm[$i]['po_quantity']);
				$gswebadm[$i]['price']=stripslashes($row_webadm[$i]['po_price']);
				$gswebadm[$i]['orderdate']=pobe_format_sort_date_2($row_webadm[$i]['ord_date']);
		...
		if(isset($total)){
			...
			$smarty->assign('showpagination',showPaginationOWD($page, $page_count, $range, $ppage, $uploadno));
...
//--------------------------------------------------------------------------------
		...
		$output=$smarty->fetch('onway_data_upload.tpl');
