...
#=================================================================
$title="Export PO - Sony AutoParts";
$GLOBALS['message']="";
#==============================================================
...
if(isset($_REQUEST['mode']))
{
	...
	{	
		$count=$obj_podata->count_partspurchased_podata_byvendor();
		...
		$ret_cur=$obj_podata->get_partspurchased_podata_byvendor($start,$limit);
		...
		if($ret_cur==1){
			...
			for($i=0;$i<count($row_webadm);$i++){
				...
				$gswebadm[$i]['vndid']=base64_encode($row_webadm[$i]['userid']);
				$gswebadm[$i]['vendorname']=pobe_siteuser_name($row_webadm[$i]['userid']);
				$gswebadm[$i]['pocnt']=pobe_count_purchase_orders($row_webadm[$i]['userid']);
...
//--------------------------------------------------------------------------------
		...
		$output=$smarty->fetch('export_po_data.tpl');
