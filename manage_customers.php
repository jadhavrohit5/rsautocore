...
#=================================================================
$title="Manage Customers Page - Sony AutoParts";
$GLOBALS['message']="";
#==============================================================
...
$obj_custdata=new customersdata;
...
{
	if($ACT=='delete' && $ID!=''){
		$custid = base64_decode($_REQUEST['id']);
		$datedeleted = date("Y-m-d H:i:s");
		$ret_val=$obj_custdata->update_customersdata(array('status'=>'0','deletedbyid'=>$wuserid,'datedeleted'=>$datedeleted,'is_deleted'=>'1'), array('cid'=>$custid));
		$smarty->assign('msg',"Record deleted successfully.");
	}

	if($_REQUEST['mode']=="show")
	{	
		$count=$obj_custdata->count_customersdata_details(array('status'=>'1'));
		...
		$ret_cur=$obj_custdata->get_customersdata($start,$limit,array('status'=>'1'));
			
		if($ret_cur==1){
			$row_reqcnt=$obj_custdata->db->sql_fetchrowset();
			...
			for($i=0;$i<count($row_reqcnt);$i++){
				...
				//	cid	name	sortcode	description	part_type	postdate	status	last_updated
				$gsreqcnt[$i]['cnt']=$artnum + $offset;
				$gsreqcnt[$i]['custid']=base64_encode($row_reqcnt[$i]['cid']);
				$gsreqcnt[$i]['customernm']=stripslashes($row_reqcnt[$i]['name']);
				$gsreqcnt[$i]['custstatus']=stripslashes($row_reqcnt[$i]['status']);
...
//--------------------------------------------------------------------------------
		...
		$output=$smarty->fetch('manage_customers.tpl');
