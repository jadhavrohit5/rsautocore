	...
	while ($rowArt = mysqli_fetch_array($sqlArt)){
		...
		if (in_array($rowArt['groupNodeId'], $ptanum) && in_array($rowArt['brandNo'], $brndno)) {
			...
			if (($rowArt['groupNodeId'] == 100551) && ($rowArt['brandNo'] == 226)) {
				...
				//$my_qry4  = "SELECT DISTINCT id,rsac,part_type FROM tbl_rsa_parts WHERE status = '1' AND is_deleted = '0' "; 
				//$my_qry4 .= " AND rsac = '". $rowArt['articleNo'] ."' AND part_type = 9 ";
...
//---------------------------------------------------------------------------------------------------------------
	...
	while ($rowBrnd = mysqli_fetch_array($sqlBrnd)){
		...
		if ($rowBrnd['part_type']==1) {
			...
			//	$updSQLb = "UPDATE tbl_rsa_app_matched_articles SET brandopt = '1' WHERE sch_id = '".addslashes($schid)."' AND carid = '".addslashes($ktypno)."' AND partid = '".addslashes($rowBrnd['partid'])."' AND status = '1' AND brandno = 377 ";
			} 
		}
