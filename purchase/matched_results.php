...
if($ret_cur==1){
	...
	for($i=0;$i<count($row_reqcnt);$i++){
		...
		$gsreqcnt[$i]['rsref']=pobe_get_group_rsac_num($row_reqcnt[$i]['partid']);
		} else {
		...
		if (($row_reqcnt[$i]['part_type'] == 14) || ($row_reqcnt[$i]['part_type'] == 15) || ($row_reqcnt[$i]['part_type'] == 16) || ($row_reqcnt[$i]['part_type'] == 17)){
			$gsreqcnt[$i]['req_qty']=$row_reqcnt[$i]['target_stock'] - pobe_group_total_stock($row_reqcnt[$i]['partid']) - - pobe_part_stock_offered($row_reqcnt[$i]['partid']) - pobe_part_stock_cart_totqnty($row_reqcnt[$i]['partid']) - pobe_part_stock_imported_qnty($row_reqcnt[$i]['partid']);         // updated on 24-03-2022 
		} else {
			$gsreqcnt[$i]['req_qty']=$row_reqcnt[$i]['target_stock'] - $row_reqcnt[$i]['a_grade'] - $row_reqcnt[$i]['b_grade'] - pobe_part_stock_offered($row_reqcnt[$i]['partid']) - pobe_part_stock_cart_totqnty($row_reqcnt[$i]['partid']) - pobe_part_stock_imported_qnty($row_reqcnt[$i]['partid']);         // updated on 24-03-2022 
		}
