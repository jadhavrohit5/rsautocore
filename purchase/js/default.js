...
//=======================================================================
 ...
 jQuery(document).ready(function ($) { 	
	...
	$( window ).scroll(function() { var y = $(this).scrollTop(); if(y>72) $("#back-top").fadeIn(); else  $("#back-top").fadeOut(); });
 	$("#back_top a").on('click', function () { smoothScroll('body', 300, 10); return false;});
  ...
  $('.cnf').on('click', function () {
   ...
   //Open rs_dialog
  $('.search_results .listing li .info .oe_info_btn').on('click', function (e) {
    e.preventDefault();
    $(this).next('.rs_dialog').toggleClass('open');
    $('body').addClass('no_scroll');
  });
  
  $(document).on('click', '.rs_dialog .overlay, .rs_dialog .close', function (e) {
    e.preventDefault();
    $('.rs_dialog').removeClass('open');
