...
$smarty->assign('partcnt',stripslashes($rowvd[0]['partcnt']));
/*--------------------------------------------------------------------------------*/
...
$count=$obj_schdata->count_regsearchdata_selected_results($schid,$carid);
...
$ret_cur=$obj_schdata->get_regsearchdata_selected_results($start,$limit,$schid,$carid);   // ,$ids
...
if($ret_cur==1){
	...
	for($i=0;$i<count($row_reqcnt);$i++){
		...
		$gsreqcnt[$i]['req_qty']=$row_reqcnt[$i]['target_stock'] - $row_reqcnt[$i]['a_grade'] - $row_reqcnt[$i]['b_grade'] - pobe_part_stock_offered($row_reqcnt[$i]['partid']) - pobe_part_stock_cart_quntity($vendorid,$wuserid,$row_reqcnt[$i]['partid']);
...
$output=$smarty->fetch('selected_articles.tpl');
