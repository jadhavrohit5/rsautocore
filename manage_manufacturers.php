...
#=================================================================
$title="Manage Car Manufacturers Page - Sony AutoParts";
$GLOBALS['message']="";
#==============================================================
...
$obj_makedata=new manufacturerdata;
...
{
	if($ACT=='delete' && $ID!=''){
		$cmid = base64_decode($_REQUEST['id']);
		$datedeleted = date("Y-m-d H:i:s");
		$ret_val=$obj_makedata->update_manufacturerdata(array('status'=>'0','deletedbyid'=>$wuserid,'datedeleted'=>$datedeleted,'is_deleted'=>'1'), array('id'=>$cmid));
		$smarty->assign('msg',"Record deleted successfully.");
	}

	if($_REQUEST['mode']=="show")
	{	
		$count=$obj_makedata->count_manufacturerdata_details(array('status'=>'1'));
		...
		$ret_cur=$obj_makedata->get_manufacturerdata($start,$limit,array('status'=>'1'));
		...
		if($ret_cur==1){
			...
			for($i=0;$i<count($row_reqcnt);$i++){
				...
				$gsreqcnt[$i]['cmid']=base64_encode($row_reqcnt[$i]['id']);
				$gsreqcnt[$i]['manufacturernm']=stripslashes($row_reqcnt[$i]['name']);
...
//--------------------------------------------------------------------------------
		...
		$output=$smarty->fetch('manage_manufacturers.tpl');
